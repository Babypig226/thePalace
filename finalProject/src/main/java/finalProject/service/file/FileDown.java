...
public class FileDown {
	public void singleFileDown(String path, String fileName, HttpServletResponse response, HttpServletRequest request) {
		String RealPath = request.getServletContext().getRealPath(path);
		String originalFileName = fileName.split("`")[1];
		String storeFileName = fileName.split("`")[0];
		String originalFileExtension = 	storeFileName.substring(storeFileName.lastIndexOf("."));
		try {
			originalFileName = URLEncoder.encode(originalFileName,"UTF-8")+originalFileExtension;
		}catch(Exception e) {}
